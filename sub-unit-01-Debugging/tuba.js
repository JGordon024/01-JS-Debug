...
function verifyAcres() {
   ...
   try{
      if(!(acresBox.value > 0)){
         throw "Please enter a number greater than zero.";
      }
   }
   catch(message){
   validity = false;
...
throw "Please enter a value that is between 1 and 12";
}
}
catch(message){
   validity=false;
   messageText=message;
   //Removes errenous entry from input box
   monthsBox.value="";
}
finally{
   monthsComplete=validity;
   //Removes former recommendations
   messageElement.innerHTML=messageText;
   messageHeadElement.innerHTML="";
   testFormCompleteness();
}
